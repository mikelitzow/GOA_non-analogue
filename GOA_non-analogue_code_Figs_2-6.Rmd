...
plot(slp$dec.yr, slp$sm.sd/100, type="l", xlab="", ylab="Standard deviation (hPa)", col="red") # converted to hPa
abline(v=(1988+11.5/12), lty=2)


dev.off()

# and plot the spatial pattern as Fig. S4

png("Fig S4.png", 8, 6, units="in", res=300)
...
# Fig. 3
Non-stationary environmental dynamics in the GOA.
...
# plot-friendly names
rownames(plot.cors) <- c("Salinity 20m", "Downwelling", "SLP gradient", "Freshwater", "Wind stress", "Advection")
rownames(load.plot) <- c("Freshwater", "SLP gradient", "Wind stress", "Advection", "Downwelling", "Salinity 20m")
...
barplot(t(var.plot), beside=T, col=c("#0072B2", "#E69F00"), ylab = "% variance", xlab="Principal component", ylim=c(0,100), names.arg = 1:6)
abline(h=0)
# legend("topright", c("Before 1988/89", "After 1988/89"), text.col=c("#0072B2", "#E69F00"), bty="n", cex=1.3)
box()
...
# Fig. 4.
Comparison of stationary and non-stationary regression models for climate-salmon catch relationships.
```{r, echo=T}
...
# and calculate residual autocorrelation for the best stationary model

mod1 <- lm(catch ~ SST, data=data) 
dwtest(resid(mod1) ~ 1)

# and best non-stationary model
mod2 <- lm(catch ~ SST*era, data=data) 
dwtest(resid(mod2) ~ 1)
```
...
```{r, echo=T}
pdf("Fig 4.pdf", 1.5*11.4/2.54, 7/2.54)
mm <- matrix(1:3, 1,3)
layout(mm, height=c(1,1),width=c(1,1))
par(las=2, mar=c(7,3,1,0.5), cex.axis= 1, tcl = 0.2, cex.lab=1, mgp=c(1.5,0.3,0), oma=c(0,0,2,0))
...
text(seq(0.5,9.9,by=1.25), -3, 
     srt = 60, adj= 1, xpd = TRUE,
     labels = models$expl, cex=0.9)

par(mgp=c(1.7,0.3,0), las=2)
...
mtext("Non-stationary models", col="#56B4E9", adj=0.15, line=0.6, outer=T)
mtext("Stationary models", col="#D55E00", adj=0.135, line=-0.8, outer=T)

par(mar=c(7,3,2,0.5))
plot(data$year, resid(mod1), type="o", pch=19, col="#D55E00", ylab="Residual", xlab="", ylim=c(-4,3))
lines(data$year, resid(mod2), type="o", pch=21, col="#56B4E9", bg="white")
abline(h=0)
lines(x=c(1988.5, 1988.5), y=c(-2,4), lty=2)
mtext("c", adj=0.11, line=-1.5, cex=1.2)
legend("bottomright", c("Stationary", "Non-stationary"), lty=c(1,1), col=c("#D55E00","#56B4E9"), pch=c(19,21), pt.bg="white", bty="n")
dev.off()
```
...
Adding this - likelihood ratio tests for hatchery and wild models.

```{r, echo=T}
wild <-  lme(catch ~  sst.3*era, random = ~ sst.3 | group,   corAR1(form = ~year|group), data=st.wild, na.action=na.exclude, control=lmeControl(opt="optim"), method="ML") 

wild.reduced <- lme(catch ~  sst.3 + era, random = ~ sst.3 | group,   corAR1(form = ~year|group), data=st.wild, na.action=na.exclude, control=lmeControl(opt="optim"), method="ML") 

wild.out <- anova(wild, wild.reduced)
wild.out
# and get exact p-value
wild.out[2,9]

# now hatchery areas
hatch <-  lme(catch ~  sst.3*era, random = ~ sst.3 | group,   corAR1(form = ~year|group), data=st.hatch, na.action=na.exclude, control=lmeControl(opt="optim"), method="ML") 

hatch.reduced <- lme(catch ~  sst.3 + era, random = ~ sst.3 | group,   corAR1(form = ~year|group), data=st.hatch, na.action=na.exclude, control=lmeControl(opt="optim"), method="ML") 

hatch.out <- anova(hatch, hatch.reduced)
hatch.out
# and get exact p-value
hatch.out[2,9]
...
# and a png version for the journal graphical abstract
png("graphical abstract.png", 6,4, units="in", res=300)
par(mgp=c(2,0.5,0), mar=c(4,4,1,1), las=1, cex.lab=1.2)

plot(dat$sst[dat$year<1989], dat$pc1[dat$year<1989], ylim=range(dat$pc1),
     xlim=range(dat$sst), pch=21, bg="#0072B2", xlab="Winter SST (°C)", ylab="Catch PC1", cex.lab=0.9)
points(dat$sst[dat$year>1988], dat$pc1[dat$year>1988], pch=21, bg="#E69F00")
lines(nd1$sst, pc.p1$fit, col="#0072B2")
lines(nd2$sst, pc.p2$fit, col="#E69F00")
...
legend(5.7, -2.5, c("Before 1988/89", "After 1988/89"), 
       text.col=c("#0072B2","#E69F00"), cex=1.3, bty="n", horiz=F)
dev.off()
```
...
for(i in 1:6){ # found an error here...changing columns 1-6 to 2-7!
 
  pink$covar <- salm.cov[match(pink$entry.yr, salm.cov$year),(i+1)] # making the change to i to match columns here
...
for(i in 1:6){
  # i <- 3
  sock$covar <- salm.cov[match(sock$entry.yr, salm.cov$year),(i+1)] # advancing i by 1 to match covariates columns
...
for(i in 1:6){
  ...
  chum$covar <- salm.cov[match(chum$entry.yr, salm.cov$year),(i+1)] # advancing i by 1 to match covariates columns
...
names(p.vals)[3:4] <- c("p.60.88", "p.89.10")
...
all.p <- c(p.vals[,3], p.vals[,4])
 ...
 means <- tapply(p.vals$p.60.88, p.vals$covar, mean) # average p-value for each covariate, across results for the 3 spp.
 means <- means[order(means)] # order low to high
 
 this <- names(means) # save the covariates in this order

 p.vals$order <- rep(c(3,2,1,6,4,5),3) # a vector of order #s to match the order in "this"

 p.vals <- arrange(p.vals, order)
 
  early <- tapply(p.vals$p.60.88,list(p.vals$covar, p.vals$species), mean) # p values for early period

  # reorder to plot
  early <- early[c(4,3,5,1,6,2),c(2,3,1)]
  
  late <- tapply(p.vals$p.89.10,list(p.vals$covar, p.vals$species), mean)

  # reorder
  late <- late[c(4,3,5,1,6,2),c(2,3,1)]
  # and plot
